...
class Time {
  ...
  isToday () {
    const now = new Date()
    return this.dateObject.toDateString() === now.toDateString()
  }
  ...
  getNow () {
    const now = Date.now()
    return now - serverTimestamp.diff
  }
  
  update () {
    this.now = this.getNow()
    this.left = (this.now - this.timestamp)
...
export default function ({ app }, inject) {
  ...
  function time (timestamp) {
    ...
    instance = new Time({ timestamp, locale })
    return instance
  }
  
  time.setServerTime = function (timestamp) {
    serverTimestamp.client = Date.now()
    serverTimestamp.server = timestamp
    serverTimestamp.diff = serverTimestamp.client - serverTimestamp.server
  }
