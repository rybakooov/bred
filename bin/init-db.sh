...
DOCKER_INIT_DB_CONFIG=${DOCKER_INIT_DB_CONFIG:-docker-compose.yml}

echo "init db config: $DOCKER_INIT_DB_CONFIG"

if  [ $(docker-compose -f $DOCKER_INIT_DB_CONFIG ps | grep db | wc -l) == 0 ]; then
